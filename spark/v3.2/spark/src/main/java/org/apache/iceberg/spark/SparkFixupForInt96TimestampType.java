...
public class SparkFixupForInt96TimestampType extends FixupTypes {
    protected SparkFixupForInt96TimestampType(Schema referenceSchema) {
        super(referenceSchema);
    }

    static Schema fixup(Schema schema) {
        return new Schema(
                TypeUtil.visit(schema, new SparkFixupForInt96TimestampType(schema)).asStructType().fields());
    }

    @Override
    public Type primitive(Type.PrimitiveType primitive) {
        if (primitive.typeId() == Type.TypeID.TIMESTAMP) {
            return Types.TimestampType.int96();
        }
        return primitive;
    }
